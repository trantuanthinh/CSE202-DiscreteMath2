...
//100 done
public class NumberOfConnectedComponentsII {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < verticesList.length; i++) {
            if (!verticesList[i].isVisited) {
                count = DFS(verticesList[i], count);
                stringBuilder.append(verticesList[i].id).append(" ").append(count);
                count = 1;
    ...
    static public int DFS(Vertex vertex, int count) {
        vertex.isVisited = true;
        for (Vertex vertex2 : vertex.adjacentVertices) {
            if (!vertex2.isVisited) {
                count = DFS(vertex2, ++count);
            }
        }
        return count;
    }
